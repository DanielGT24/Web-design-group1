<?php 

session_start();

	include("connection.php");
	include("functions.php");

	$user_data = check_login($con);

?>


<!DOCTYPE html>
<html lang="en">
<head>
<title>Formula Fable</title>
<meta charset="utf-8">
<link rel="stylesheet" href="Grand Prix Info.css">
<link rel="icon" type="image/x-icon" href="Formula.png">

<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
  
</style>
</head>
<body>
  <div class="lo">
    <div style="display:inline-block;vertical-align:top;">
      <a href ="index.php"> <img src="Formula.png" width="180" height="110"/> </a>
    </div>
        <div style="display:inline-block;">

    <h1> 
      <div class="topnav">
  <a href ="index.php">Home</a>
  <a href="F1 History.php">History</a>
  <a href="Constructors.php">Constructors</a>
  <a href="Drivers.php">Drivers</a>
   <a href="Grand Prix 2022.php">Grand Prix 2022</a>
    <a href="Gallery.php">Gallery</a> 
    <div class="dropdown">
      <button class="dropbtn">Other</button>
      <div class="dropdown-content">
      <a href="About.php">About Us</a>
        <a href="Contact.html">Contact Us</a>
        <a href="logout.php">Log Out</a>
      </div>
      </div>
    </div> 
  </h1>
 

   
</div>
</div>

  </div> 
<body>


    <section>
      
      
      <article>
        <h1 style = "text-align: center;font-size: 40px;"> GRAND PRIX 2022</h1>
       <hr>
   </div>

<div style="display:inline-block;vertical-align:top;">
    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/88/Flag_of_Australia_%28converted%29.svg/1200px-Flag_of_Australia_%28converted%29.svg.png" width="105" height="80"/> 
</div>
    <div style="display:inline-block;">
<div class="header">
  <h1 style = "font-family: 'Courier New', Courier, monospace; font-size: 30px;"> Formula 1 Heineken Australian</h1>

</div>
</div>
      
            <img style= "float: right;" src="https://cdn-1.motorsport.com/static/img/amp/5300000/5320000/5321000/5321100/5321167/s6_1000549/1000549.jpg"
             width="500" height="300"/>
            </div>
      <h1 style = "font-family: 'Courier New', Courier, monospace; font-size: 30px;"> Australia Grand Prix</h1>
               
                <P> Formula 1 Heineken Australian Grand Prix 2022) was a Formula One motor race that was held on 10 April 2022 in Melbourne, Victoria. It was contested at the Albert Park Circuit and was the third round of the 2022 Formula One World Championship. &nbsp; 
                    &nbsp;   
 &nbsp;  </P>  
       
         <p></p>
            
             </div> 


            </div>
            <br>

   


          </div>
       
             <p>&nbsp; </p>
      
         
       
         <hr>
          <div style="display:inline-block;vertical-align:top;">
            <img  src="https://www.formula1.com/content/dam/fom-website/2018-redesign-assets/Circuit%20maps%2016x9/Australia_Circuit.png.transform/9col/image.png" width="800" height="500"/> 
        </div>
            <div style="display:inline-block;">
        <div class="para">
         <h1 style = "font-family: 'Courier New', Courier, monospace; font-size: 30px;"> Australia Grand Prix Circuit</h1>
            <ul class = "no-bullets"> 
             <li Style = "text-align:left"><h2>Albert Park Circuit</h2></li>
             <P></P>
             <li Style = "text-align:left">Temporary street circuit</li>
             <P></P>
             <li Style = "text-align:left">Course Length: 5.278 km (3.280 miles)</li>
             <P></P>
             <li Style = "text-align:left">Distance: 58 laps, 306.124 km (190.216 miles)</li>
             <P></P>
             <li Style = "text-align:left">Weather:  Partly cloudy</li>
             <P></P>
             <li Style = "text-align:left">Coordinates: 37°50′59″S 144°58′6″E</li>
             <P></P>            
             <li Style = "text-align:left">Opened: 20 November 1953</li>
 
           </ul>
        </div>
        </div>
<hr>
        <br>

     
  <div class="container">
    <div class="image">
          <img src="https://s3-eu-west-1.amazonaws.com/racingnews-v2-prod/2022/Leclerc/_1125x633_crop_center-center_85_none/220016-australian-gp-free-practice-report.jpg?v=1649473940"
  width="700" height="400"/>
          </div>
   
          <div class = "text"> <h1 style = "font-family: 'Courier New', Courier, monospace; font-size: 30px;"> Australia Grand Prix Race  </h1>
             
              <P>The race took place at 15:00 local time on 10 April and lasted 58 laps. </p>
               <P>Ferrari's Charles Leclerc scored his first career grand slam, having started in pole position, set the fastest lap, led every lap,</p>
                 <P>Leclerc won the race ahead of Red Bull's Sergio Pérez and Mercedes' George Russell.
                 </P>
                 <p>Leclerc maintained a lead ahead of Verstappen who challenged him most of the race until Verstappen retired due to a fuel issue causing a fire in his car
                    </p>
                 <P> This turn of events secures Leclerc's win </P> 
                 <P> Charles Leclerc had the fastest lap at time: 1:20.260 on lap 58.</P>
                 &nbsp; 
  
               
           </div> 


          </div>
 <hr>
 &nbsp; 
 &nbsp; 
</div>
    </div>
   
   <div class="center">
   <img src="https://royalcoastreview.com/wp-content/uploads/2022/04/GP-1.jpg"
    width="700" height="400"/><h1 style = "text-align: center;font-family: 'Courier New', Courier, monospace; font-size: 30px;">  2022 AUSTRALIA GRAND PRIX PODIUM</h1>
</div>
&nbsp;
&nbsp;
&nbsp;
&nbsp;

 <h1 style = "text-align: center;font-family: 'Courier New', Courier, monospace; font-size: 30px;"> RACE RESULTS</h1>

   <table>
    <tr>
      <th>pos.</th>
      <th>no.</th>
      <th>driver</th>
      <th>constructor</th>
      <th>laps</th>
      <TH>Time/Retired</TH>
      <th>pts</th>
    </tr>
   
   <tr>
      <td>1</td>
      <td>16</td>
      <td> <img src ="https://upload.wikimedia.org/wikipedia/commons/thumb/e/ea/Flag_of_Monaco.svg/1280px-Flag_of_Monaco.svg.png" height = "15" width=" 25"> Charles Leclerc</td>
      <td>FERRARI</td>
      <td>58</td>
      <td>1:27:46.548</td>
      <td>26</td>
    </tr> 
     <tr>
      <td>2</td>
      <td>11</td>
      <td><img src ="https://cdn.britannica.com/73/2573-004-29818847/Flag-Mexico.jpg" height = "15" width=" 25"> Sergio Perez</td>
      <td> RED BULL RACING-RBPT</td>
      <td>58</td>
      <td>+20.524</td>
      <td> 18</td>
    </tr>  
     <tr>
      <td>3</td>
      <td>63</td>
      <td><img src ="https://upload.wikimedia.org/wikipedia/commons/thumb/2/2c/Flag_of_the_United_Kingdom_%282-3%29.svg/1200px-Flag_of_the_United_Kingdom_%282-3%29.svg.png" height = "15" width=" 25"> George Russell</td>
      <td> MERCEDES</td>
      <td>58</td>
      <td>+25.593</td>
      <td> 15</td>
    </tr>
    <tr>
        <td>4</td>
        <td>44</td>
        <td><img src ="https://upload.wikimedia.org/wikipedia/commons/thumb/2/2c/Flag_of_the_United_Kingdom_%282-3%29.svg/1200px-Flag_of_the_United_Kingdom_%282-3%29.svg.png" height = "15" width=" 25"> Lewis Hamilton</td>
        <td> MERCEDES</td>
        <td>58</td>
        <td>+28.543</td>
        <td> 12</td>
      </tr>
  
      <tr>
        <td>3</td>
        <td>44</td>
        <td><img src ="https://upload.wikimedia.org/wikipedia/commons/thumb/2/2c/Flag_of_the_United_Kingdom_%282-3%29.svg/1200px-Flag_of_the_United_Kingdom_%282-3%29.svg.png" height = "15" width=" 25"> Lando Norris</td>
        <td>MCLAREN-MERCEDES</td>
        <td>58</td>
        <td>+53.303</td>
        <td> 10</td>
      </tr>
  
  
 
    </tr>
  </table>
  

   </article>
    </section>

    <footer>
   
    
      <ul class="ul">
       
    
        <a href="#"><li  class="li"></li></a>
      <a href="https://www.instagram.com/formula_fable/?hl=en"><li  class="li">  <img src="https://i.pinimg.com/originals/64/c7/c8/64c7c86fd70cb2aea77b49fd340164b3.webp" class="image" width="32" height="32" ></li></a>
      <a href="https://twitter.com/FableFormula"><li class="li"><img src="data:image/png;base64," class="image" width="30" height="30"></li></a>
     
        <h3>Copyright © 2022 Formula Fable</h3>
    </ul>
    
    </footer>

 
   
    </body>
    <button onclick="topFunction()" id="myBtn" title="Go to top">Top</button>

<script src="Top.js">  
</script> 

  
</body>
</html>


