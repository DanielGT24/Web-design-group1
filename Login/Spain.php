<?php 

session_start();

	include("connection.php");
	include("functions.php");

	$user_data = check_login($con);

?>


<!DOCTYPE html>
<html lang="en">
<head>
<title>Formula Fable</title>
<meta charset="utf-8">
<link rel="stylesheet" href="Grand Prix Info.css">
<link rel="icon" type="image/x-icon" href="Formula.png">

<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
  
</style>
</head>
<body>
  <div class="lo">
    <div style="display:inline-block;vertical-align:top;">
      <a href ="index.php"> <img src="Formula.png" width="180" height="110"/> </a>
    </div>
        <div style="display:inline-block;">

    <h1> 
      <div class="topnav">
  <a href ="index.php">Home</a>
  <a href="F1 History.php">History</a>
  <a href="Constructors.php">Constructors</a>
  <a href="Drivers.php">Drivers</a>
   <a href="Grand Prix 2022.php">Grand Prix 2022</a>
    <a href="Gallery.php">Gallery</a> 
    <div class="dropdown">
      <button class="dropbtn">Other</button>
      <div class="dropdown-content">
      <a href="About.php">About Us</a>
        <a href="Contact.html">Contact Us</a>
        <a href="logout.php">Log Out</a>
      </div>
      </div>
    </div> 
  </h1>
 

   
</div>
</div>

  </div> 
<body>


    <section>
      
      
      <article>

        <h1 style = "text-align: center;font-size: 40px;"> GRAND PRIX 2022</h1>
        
       <hr>
   </div>

<div style="display:inline-block;vertical-align:top;">
    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Flag_of_Spain.svg/2560px-Flag_of_Spain.svg.png"  width="105" height="80"/> 
</div>
    <div style="display:inline-block;">
<div class="header">
  <h1 style = "font-family: 'Courier New', Courier, monospace; font-size: 30px;"> Formula 1 Pirelli Gran Premio de España 2022 </h1>

</div>
</div>
      
            <img style= "float: right;" src="https://cdn-wp.thesportsrush.com/2022/05/efd45e2f-untitled-design.jpg"
           width="500" height="350"/>
            </div>
      <h1 style = "font-family: 'Courier New', Courier, monospace; font-size: 30px;"> Spanish Grand Prix</h1>
               
                <P> Formula 1 Pirelli Gran Premio de España 2022 was a Formula One motor race held on 22 May 2022 at the Circuit de Barcelona-Catalunya in Montmeló, Spain. It was the 62nd edition of the Spanish Grand Prix and the sixth round of the 2022 Formula One World Championship.
 &nbsp;  </P>  
       
         <p></p>
            
             </div> 


            </div>
            <br>

   
 &nbsp; 
          &nbsp;
            
   
          </div>
         
      
          <p>&nbsp; </p>
     
         <hr>
          <div style="display:inline-block;vertical-align:top;">
            <img  src="https://www.formula1.com/content/dam/fom-website/2018-redesign-assets/Circuit%20maps%2016x9/Spain_Circuit.png.transform/9col/image.png"  width="800" height="500"/> 
        </div>
            <div style="display:inline-block;">
        <div class="para">
         <h1 style = "font-family: 'Courier New', Courier, monospace; font-size: 30px;"> Spanish Grand Prix Circuit</h1>
            <ul class = "no-bullets"> 
             <li Style = "text-align:left"><h2>Circuit de Barcelona-Catalunya</h2></li>
             <P></P>
             <li Style = "text-align:left">Permanent racing facility</li>
             <P></P>
             <li Style = "text-align:left">Course Length: 4.675 km (2.905 miles)</li>
             <P></P>
             <li Style = "text-align:left">Distance:     66 laps, 308.424 km (191.646 miles</li>
             <P></P>
             <li Style = "text-align:left">Weather: Sunny </li>
             <P></P>
             <li Style = "text-align:left">Coordinates: 41°34′12″N 2°15′40″E</li>
             <P></P>            
             <li Style = "text-align:left">Opened:10 September 1991</li>
 
           </ul>
        </div>
        </div>
<hr>
        <br>

     
  <div class="container">
    <div class="image">
          <img src="https://www.formula1.com/content/dam/fom-website/sutton/2018/Spain/Sunday/dcd1813my2211.jpg"
          width="700" height="400"/>
          </div>
   
          <div class = "text"> <h1 style = "font-family: 'Courier New', Courier, monospace; font-size: 30px;">Spanish Grand Prix Race  </h1>
             
              <P>The race started at 15:00 local time and lasted 66 laps.</p>
               <P> Max Verstappen won the race ahead of teammate Sergio Pérez, while George Russell completed the podium</p>
                    <P> Charles Leclerc, the championship leader, was forced to retire due to engine issues</P> 
                    <P> Charles Leclerc kept the lead ahead of Max Verstappen until the MGU-H failure  </P>
                    <p>This resulted in Verstappen securing the lead and winning the race</p>
                 <P> Sergio Pérez had the fastest lap at time: 1:24.108 on lap 55</P>
                 &nbsp; 
  
               
           </div> 


          </div>
 <hr>
 &nbsp; 
 &nbsp; 
</div>
    </div>
   
   <div class="center">
   <img src="https://www.gannett-cdn.com/presto/2022/05/08/NPPP/7167c573-4840-4979-8593-530593e33c6f-050822_F1_Miami1899.JPG"  width="700" height="400"/><h1 style = "text-align: center;font-family: 'Courier New', Courier, monospace; font-size: 30px;">  2022 SPANISH GRAND PRIX PODIUM</h1>
</div>
&nbsp;
&nbsp;
&nbsp;
&nbsp;

 <h1 style = "text-align: center;font-family: 'Courier New', Courier, monospace; font-size: 30px;"> RACE RESULTS</h1>

   <table>
    <tr>
      <th>pos.</th>
      <th>no.</th>
      <th>driver</th>
      <th>constructor</th>
      <th>laps</th>
      <TH>Time/Retired</TH>
      <th>pts</th>
    </tr>
    <tr>
        <td>1</td>
        <td>1</td>
        <td><img src ="https://upload.wikimedia.org/wikipedia/commons/thumb/2/20/Flag_of_the_Netherlands.svg/1200px-Flag_of_the_Netherlands.svg.png" height = "15" width=" 25">Max Verstappen</td>
        <td>RED BULL RACING-RBPT</td>
        <td>66</td>
        <td>1:37:20.475</td>
        <td> 25</td>
      </tr>  
           <tr>
        <td>2</td>
        <td>11</td>
        <td><img src ="https://cdn.britannica.com/73/2573-004-29818847/Flag-Mexico.jpg" height = "15" width=" 25"> Sergio Perez</td>
        <td> RED BULL RACING-RBPT</td>
        <td>66</td>
        <td>+13.072</td>
        <td> 19</td>
      </tr>  
     
      <tr>
        <td>2</td>
        <td>66</td>
        <td><img src ="https://upload.wikimedia.org/wikipedia/commons/thumb/2/2c/Flag_of_the_United_Kingdom_%282-3%29.svg/1200px-Flag_of_the_United_Kingdom_%282-3%29.svg.png" height = "15" width=" 25"> George Russell</td>
        <td> MERCEDES</td>
        <td>63</td>
        <td>+32.927</td>
        <td> 15</td>
      </tr>  
          <tr>
        <td>4</td>
        <td>55</td>
        <td> <img src ="https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Flag_of_Spain.svg/2560px-Flag_of_Spain.svg.png" height = "15" width=" 25"> Carlos Sainz Jr.</td>
        <td> FERRARI</td>
        <td>66</td>
        <td>+45.208</td>
        <td>12</td>
      </tr>

      <tr>
        <td>5</td>
        <td>44</td>
        <td><img src ="https://upload.wikimedia.org/wikipedia/commons/thumb/2/2c/Flag_of_the_United_Kingdom_%282-3%29.svg/1200px-Flag_of_the_United_Kingdom_%282-3%29.svg.png" height = "15" width=" 25"> Lewis Hamilton</td>
        <td> MERCEDES</td>
        <td>66</td>
        <td>    +54.534</td>
        <td> 10</td>
      </tr>
 

    </tr>
  </table>
  

   </article>
    </section>
  
    <footer>
   
    
      <ul class="ul">
       
    
        <a href="#"><li  class="li"></li></a>
      <a href="https://www.instagram.com/formula_fable/?hl=en"><li  class="li">  <img src="https://i.pinimg.com/originals/64/c7/c8/64c7c86fd70cb2aea77b49fd340164b3.webp" class="image" width="32" height="32" ></li></a>
      <a href="https://twitter.com/FableFormula"><li class="li"><img src="data:image/png;base64," class="image" width="30" height="30"></li></a>
     
        <h3>Copyright © 2022 Formula Fable</h3>
    </ul>
    
    </footer>

 
   
    </body>
    <button onclick="topFunction()" id="myBtn" title="Go to top">Top</button>

<script src="Top.js">  
</script> 

  
</body>
</html>


 