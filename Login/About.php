<?php 

session_start();

	include("connection.php");
	include("functions.php");

	$user_data = check_login($con);

?>


<!DOCTYPE html>
<html lang="en">
<head>
<title>Formula Fable</title>
<meta charset="utf-8">
<link rel="stylesheet" href="About.css">
<link rel="icon" type="image/x-icon" href="Formula.png">

<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
  
</style>
</head>
<body>
  <div class="lo">
    <div style="display:inline-block;vertical-align:top;">
      <a href ="index.php"> <img src="Formula.png" width="180" height="110"/> </a>
    </div>
        <div style="display:inline-block;">

    <h1> 
      <div class="topnav">
  <a href ="index.php">Home</a>
  <a href="F1 History.php">History</a>
  <a href="Constructors.php">Constructors</a>
  <a href="Drivers.php">Drivers</a>
   <a href="Grand Prix 2022.php">Grand Prix 2022</a>
    <a href="Gallery.php">Gallery</a> 
    <div class="dropdown">
      <button class="dropbtn">Other</button>
      <div class="dropdown-content">
      <a href="About.php">About Us</a>
        <a href="Contact.html">Contact Us</a>
        <a href="logout.php">Log Out</a>
      </div>
      </div>
    </div> 
  </h1>
 

   
</div>
</div>

  </div> 
<body>


    <section>
      
      
      <article>

        <h1 style = "text-align: center;font-size: 40px;"> ABOUT US</h1>
        
       <hr>
   </div>
   <center><img src="Formula.png" alt="img"
    alt=Bahrain width="305" height="200"/> </center>
   <h2 style="text-align: center;">Formula Fable</h2>
   <p style="text-align: justify;font-size: 20px;padding-left: 200px;padding-right: 200px;"> For the past 10 years, Formula Fable has delivered the latest information about Formula 1 Racing:
including it's history and it's minitial conception in the 1950's.
 It's implementation into the different leagues of FIA Racing.
 The winners and losers over the years and its impact
upon the world as an automotive and medical scene.
 We focus on delivering the latest Standings on F1 constructors & drivers
And the weekly Grand Prix as it is held all over the world. </p>
<h2 style="text-align: center;font-size: 20px;padding-left: 200px;padding-right: 200px;">Welcome to Formula Fable. </h2>


   </article>
    </section>
  
    <footer>
   
    
      <ul class="ul">
       
    
        <a href="#"><li  class="li"></li></a>
      <a href="https://www.instagram.com/formula_fable/?hl=en"><li  class="li">  <img src="https://i.pinimg.com/originals/64/c7/c8/64c7c86fd70cb2aea77b49fd340164b3.webp" class="image" width="32" height="32" ></li></a>
      <a href="https://twitter.com/FableFormula"><li class="li"><img src="data:image/png;base64," class="image" width="30" height="30"></li></a>
     
        <h3>Copyright © 2022 Formula Fable</h3>
    </ul>
    
    </footer>

 
   
    </body>
    <button onclick="topFunction()" id="myBtn" title="Go to top">Top</button>

    <script src="Top.js">  
    </script> 
  
</body>
</html>


 